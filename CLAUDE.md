...
- `GoalsPage` and `ReportsPage` are still placeholders ("em construção") — routed (`/app/goals`, `/app/reports`) but not yet wired to the backend Goals/Reports modules that already exist. `components/GoalsList.tsx` still renders hardcoded fake data.
