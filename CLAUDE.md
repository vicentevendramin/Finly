...
**Note:** this project is mid-refactor per `PLAN.md` at the repo root — the entire backend (NestJS/TypeORM, all modules incl. Goals/Reports/Admin/Observability), the Docker/Prometheus/Grafana infra, and the entire frontend feature set (router, TanStack Query, Zustand, i18n, design system, Goals/Reports/Admin pages) are done. Only frontend automated tests (F8, optional/unconfirmed scope) and a production frontend Dockerfile (I3) remain open. Check `PLAN.md`'s task table before assuming a section is finished.
...
- Vite `base` is `/` (self-hosted via Docker/nginx, not GitHub Pages); `VITE_API_URL` env var overrides the default `http://localhost:3001/api` backend URL. No frontend tests yet (F8 in `PLAN.md` is still open, scope unconfirmed) — changes were verified with a scripted headless-browser walkthrough instead (see git history), not an automated suite.
